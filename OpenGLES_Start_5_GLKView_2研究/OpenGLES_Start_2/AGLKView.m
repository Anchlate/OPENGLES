...
+ (Class)layerClass {
    ...
    return [CAEAGLLayer class];// CAEAGLLayer 是 Core Animation提供的标准层之一。 CAEAGLLayer会与一个OpenGL ES 的帧缓存共享它的像素颜色仓库
}
...
- (id)initWithFrame:(CGRect)frame context:(EAGLContext *)aContext {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        eaglLayer.drawableProperties = [NSDictionary dictionaryWithObjectsAndKeys:
                                        ...
                                        [NSNumber numberWithBool:NO], kEAGLDrawablePropertyRetainedBacking, // 不保存以前绘制的图像
                                        kEAGLColorFormatRGBA8, kEAGLDrawablePropertyColorFormat, // 用8位来保存每个像素的每个颜色元素的值
                                        nil];
