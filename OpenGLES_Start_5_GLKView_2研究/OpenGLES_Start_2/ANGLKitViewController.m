...
- (void)viewDidLoad {
    ...
    self.baseEffect.constantColor = GLKVector4Make(1.0, 1.0, 1.0, 1.0); // 渲染像素颜色（三角形的颜色）
    ...
    //第一个参数用于指定要更新当前上下文中所绑定的时哪一个缓存。（这里是定点缓存）
...
- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect {
    ...
    [self.baseEffect prepareToDraw];
     ...
     // 当前绑定的帧缓存的像素颜色缓存中的每一个像素的颜色为当前 glClearColor() 函数设定的值。
