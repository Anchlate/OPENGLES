...
typedef struct {
    ...
    GLKVector3 positionCoords;// 顶点位置
...
- (void)viewDidLoad {
    ...
    // 重新给view创建一个 OpenGL ES 2.0 的上下文
    ...
    // 创建一个纹理缓存
    // option: 接受了一个存储了用于指定GLKTextureLoader怎么解析加载的图像数据的键值对的NSDictionary，例如：指示GLKTextureLoader为加载的图像生成MIP图像
    GLKTextureInfo *textureInfo = [GLKTextureLoader textureWithCGImage:imageRef options:nil error:NULL];
    
    // 设置GLKBaseEffect来使用一个新的纹理换粗
    self.baseEffect.texture2d0.name = textureInfo.name;
    self.baseEffect.texture2d0.target = textureInfo.target;
    
//    NSLog(@",,,,,,,,,,texttureInfo name: %d", textureInfo.name);
//    NSLog(@"..........texttureInfo target: %d", textureInfo.target);
}
