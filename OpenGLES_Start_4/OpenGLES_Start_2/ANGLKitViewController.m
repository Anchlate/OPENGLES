...
- (void)viewDidLoad {
    ...
    //GL_STATIC_DRAW 提示告诉上下文，缓存中的内容适合复制到GPU控制的内存，因为很少对其进行修改。这个信息可帮助OpenGL ES优化内存使用。
    //GL_DYNAMIC_DRAW  提示数据会频繁改变，同时提示ES 以不同的方式处理缓存中的存储。
...
- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect {
    ...
    //最后一个参数 NULL，这告诉OpenGL ES可以从当前绑定的顶点缓存的开始位置访问定点数据。
